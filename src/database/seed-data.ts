...
/**
 * Helper to parse a line of CSV handling quotes and delimiters
 */
...
/**
 * Dynamically search for a CSV file in `contoh` or `contoh/data` by keyword
 */
...
/**
 * 1. Load Master Wilayah from CSV
 */
...
async function loadUsersFromCSV(userRepo: any): Promise<User[]> {
  ...
  // Ensure mandatory default admin users exist
...
/**
 * 4. Load Data Pengaturan from CSV
 */
...
/**
 * 5. Load Data Pelanggan (all 8,368+ records) from CSV
 */
  ...
  if (csvPath) {
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Generate Invoices for Maret & Februari 2026
        invoiceList.push(
          invoiceRepo.create({
...
export async function seedInitialData(dataSource: DataSource) {
  ...
  // Drop and recreate clean database tables with updated entity schemas
  try {
    ...
    // Synchronize fresh tables according to entity definitions
  ...
  // 5. Seed Data Pelanggan & Invoices
